...
pub trait BiFunctorOnce<A, B, Tag = ()> : BiTypeMap<A, B, Tag> {
    fn fmap_once2<T, U>(self, f: impl FnMut(A) -> T, g: impl FnMut(B) -> U)
                        -> Self::BiFunctor<T, U>;
}
...
impl<A, B> BiFunctorOnce<A,B> for (A, B) {
    fn fmap_once2<T,U>(self, mut f: impl FnMut(A)->T, mut g: impl FnMut(B)->U)
                       -> (T, U) {
        (f(self.0), g(self.1))
    }
}
...
impl<A, B, C: BiFunctorOnce<A, B>> FunctorOnce<A, Proj0<B>> for C {
    fn fmap_once<T>(self, f: impl FnMut(A) -> T)
                    -> <Self as BiTypeMap<A, B>>::BiFunctor<T, B> {
        self.fmap_once2(f, |y| y)
    }
}
...
impl<A, B, C: BiFunctorOnce<A, B>> FunctorOnce<B, Proj1<A>> for C {
    fn fmap_once<T>(self, g: impl FnMut(B) -> T)
                    -> <Self as BiTypeMap<A, B>>::BiFunctor<A, T> {
        self.fmap_once2(|x| x, g)
    }
}
...
fn bif() {
    ...
    let r = p.fmap_once2(|x| x*2, |y| y as f32 / 2.0);
    assert_eq!(r, (8, 2.5));
